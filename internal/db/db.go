...
func (d *DB) GetFingerprintsByHost(hostID int64) ([]string, error) {
	rows, err := d.db.Query(`SELECT DISTINCT technology FROM fingerprints WHERE host_id=?`, hostID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (d *DB) GetDashboardStats() (*models.DashboardStats, error) {
	...
	d.db.QueryRow(`SELECT COUNT(*) FROM tech_findings`).Scan(&s.TechFindings)
	d.db.QueryRow(`SELECT COUNT(*) FROM cloud_findings`).Scan(&s.CloudFindings)
...
func (d *DB) GetTechFindings(domain, tech, severity string, limit, page int) ([]map[string]interface{}, int, error) {
	where, args := "1=1", []interface{}{}
	if domain != "" {
		where += " AND domain=?"
		args = append(args, domain)
	...
	rows, err := d.db.Query(fmt.Sprintf(
		...
		last_seen DESC LIMIT ? OFFSET ?`, where),
		append(args, limit, offset)...,
	)
	if err != nil {
		return nil, 0, err
...
func (d *DB) GetCloudFindings(domain, provider, severity string, limit, page int) ([]map[string]interface{}, int, error) {
	where, args := "1=1", []interface{}{}
	if domain != "" {
		where += " AND domain=?"
		args = append(args, domain)
...
func (d *DB) WipeDomain(domain string) error {
	...
	tx.Exec(`DELETE FROM tech_findings WHERE domain=?`, domain)
	tx.Exec(`DELETE FROM cloud_findings WHERE domain=?`, domain)
	tx.Exec(`DELETE FROM alert_history WHERE domain=?`, domain)
	return tx.Commit()
}
...
func (d *DB) WipeAll() error {
	...
	defer tx.Rollback()
	for _, t := range []string{"js_findings", "fingerprints", "ports", "hosts", "assets", "change_events", "scans", "tech_findings", "cloud_findings", "alert_history"} {
...
func (d *DB) ExportDomain(domain string) (map[string]interface{}, error) {
	...
	techFin, _, _ := d.GetTechFindings(domain, "", "", 100000, 1)
	cloudFin, _, _ := d.GetCloudFindings(domain, "", "", 100000, 1)
	...
	return map[string]interface{}{
		...
		"ports":          ports,
		"js_findings":    jsFin,
		"tech_findings":  techFin,
		"cloud_findings": cloudFin,
		"changes":        changes,
	}, nil
}
