...
func New(cfg *config.Config, database *db.DB, logger func(string, ...interface{})) *Processor {
	...
	alertMgr := alerting.New(
		...
		alerting.AlertConfig{
			...
			CloudExposure:   cfg.Alerting.AlertOn.CloudExposure,
		},
		logger,
	)
...
func (p *Processor) RunScan(ctx context.Context, domain, profileName string) (*ScanResult, error) {
	...
	for _, hr := range httpxResults {
		...
		// Check for new technologies (compare against previously stored fingerprints)
		if hostID > 0 {
			prevTechs, _ := p.db.GetFingerprintsByHost(hostID)
			prevSet := make(map[string]bool)
			for _, pt := range prevTechs {
				prevSet[strings.ToLower(pt)] = true
			}
			for _, tech := range hr.Technologies {
				name, ver := parseTech(tech)
				if name != "" && !prevSet[strings.ToLower(name)] {
